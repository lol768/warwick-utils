...
<head>
	...
	Event.onDOMReady(function() {
		addEvent(document.getElementById('playVideo_${uniqueId}'), 'click', function(evt) {
			...
			// do not use popout for smallscreen devices
			if(document.getElementsByTagName('body')[0].className.indexOf('is-smallscreen') >= 0) {
				return true;
			}
			...
			iframe.setAttribute('width', '<@dimension value=width?default(640) />');
			iframe.setAttribute('height', '<@dimension value=height?default(360) />');
			...
			container.style.marginLeft = '-' + Math.round(container.scrollWidth / 2) + 'px';
			container.style.marginTop = '-' + Math.round(container.scrollHeight / 2) + 'px';
