...
class RedCloth < String
    ...
    def to_html( *rules )
        ...
        textile_rules = [:refs_textile, :block_textile_table, :block_textile_lists,
                         ...
                         :inline_textile_autolink_urls, :inline_textile_autolink_urls, :inline_textile_autolink_emails # do these twice to accomodate for one space/line diffs
                         ]
	...
	#define a random number to put on footnotes
	@random = rand(100000)
        ...
        check_link_replace_tildes_in_urls text
        inline text
        smooth_offtags text
    ...
    def hard_break( text )
        # keep the linebreak after the <br /> tag to maintain readability
        text.gsub!( /(.)\n(?!\n|\Z| *([#*=]+(\s|$)|[{|]))/, "\\1<br />\n" ) if hard_breaks
    end
        ...
        text.replace( text.split( BLOCKS_GROUP_RE ).collect do |blk|
            ...
            if blk =~ /^<\/?(\w+).*>/ and not SIMPLE_HTML_TAGS.include? $1
                ...
                if blk.empty?
                    ...
                    blk.gsub!( /((?:\n(?:\n^ +[^\n]*)+)+)/m ) do |iblk|
                        flush_left iblk
                        blocks iblk#, plain #stop putting pre/code blocks around things - uncomment the end of this to re-enable
                        iblk.gsub( /^(\S)/, "\t\\1" )
                        if plain
                            code_blk = iblk; ""
                        else
    ...
    def check_link_replace_tildes_in_urls(text)
    	text.gsub!(AUTO_LINK_RE) do
        	all, a, b, c, d = $&, $1, $2, $3, $5
        	if a =~ /<a\s/i # don't replace URL's that are already linked
          		all
    ...
    # These entities will be escaped when they have a single backslash before them
    # 
    # They are, at the moment, replaced with XML entities
    QTAGS_ESCAPE = [
    ['**', '&#0042;&#0042;'], 
    ['*', '&#0042;'], 
    ['??', '&#0063;&#0063;'],
    ...
    #replace single-escape then entity with entity's xml equivalent
    def escape_qtags (text)
      QTAGS_ESCAPE.each do |qtag_rc, entity|
        re = Regexp::quote qtag_rc
        qtag_re = /([^\\])\\#{re}/
        text.gsub!( qtag_re ) do |m|
